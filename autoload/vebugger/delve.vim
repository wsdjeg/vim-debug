...
function! vebugger#delve#start(entryFile,args)
    ...
    call l:debugger.writeLine(vebugger#util#WinShellSlash(a:entryFile) . ' ; exit')
...
function! vebugger#powershell#_requestEvaluateExpression(writeAction,debugger)
    for l:evalAction in a:writeAction
        call a:debugger.writeLine('$'.l:evalAction.expression)
    endfor
endfunction
...
function! vebugger#powershell#_readEvaluatedExpressions(pipeName,line,readResult,debugger) dict
    if 'out'==a:pipeName
        if has_key(self,'expression') "Reading the actual value to print
            ...
            " fuck the ps expression
            " Linux
            " [DBG]: PS /home/jen/debug>> $fahrenheit
            " isWindows
            " [DBG]: PS C:\Users\wsdjeg\SpaceVim\vim-debug>> $name
            if s:SYS.isWindows
                " At C:\Users\wsdjeg\Desktop\test.ps1:1 char:1 
                let l:expression=matchstr(a:line,'\(\[DBG\]\: PS\s[^>]\+>> \$\)\@<=.*')
            else
                let l:expression=matchstr(a:line,'\v^\(Pdb\) p (.*)$')
            endif

            if 1<len(l:expression)
                let self.expression=l:expression
            endif
        endif
    endif
endfunction
